...
TextureAtlas.CASHE_STATE_CACHED   = 0; // キャッシュ更新済み
TextureAtlas.CASHE_STATE_PROGRESS = 1; // キャッシュ更新中
TextureAtlas.CASHE_STATE_DIRTY    = 2; // キャッシュが汚れている。更新が必要
TextureAtlas.CLASSIFY_FIND_FIRST  = 0;
TextureAtlas.CLASSIFY_COMPACT     = 1;
TextureAtlas.CLASSIFY_ALIGN       = 2;
...
TextureAtlas["prototype"] = Object.create(TextureAtlas, {
    ...
    "add":          { "value": TextureAtlas_add         },  // TextureAtlas#add(resources:ResourceArray, group:String = "", classify:Integer = 0):void
...
//}@dev
    ...
    if (data) {
        ...
        var res = this._spriteSheets[ssn]; // HTMLCanvasElement or HTMLImageElement

        if (this._useCache) {
            switch (this._cacheState[ssn]) {
            case TextureAtlas.CASHE_STATE_CACHED: res = this._imageCache[ssn]; break;
...
function TextureAtlas_updateCache() {
    if (this._useCache) {
        var ssna = Object.keys(this._cacheState); // [ssn, ...]
...
function _updateCache(that,    // @arg this
    ...
    that._cacheState[ssn] = TextureAtlas.CASHE_STATE_PROGRESS; // DIRTY -> PROGRESS
    ...
    _toBlob(that._spriteSheets[ssn], "image/png", async, function(blob) {
        ...
        that._cacheState[ssn] = TextureAtlas.CASHE_STATE_CACHED; // PROGRESS -> CACHED
...
function TextureAtlas_add(resources,  // @arg ResourceArrayArray|ResourceObjectArray - [[source, id, rect ] or { source, id, rect }, ...]
                          ...
                          classify) { // @arg Integer = FIND_FIRST - classify mode (FIND_FIRST, COMPACT, ALIGN)
                                      // @resource.id     String
...
//{@dev
    if (!global["BENCHMARK"]) {
        ...
        $valid($type(classify,  "Integer|omit"), TextureAtlas_add, "classify");
    }
//}@dev
    ...
    for (var i = 0, iz = resources.length; i < iz; ++i) {
        ...
        var bs = _getFitBlockSize(sw, sh); // block size
        var candidates = []; // [ssn, ...]

        switch (classify) {
        case TextureAtlas.CLASSIFY_FIND_FIRST: // 最初に見つかった場所に設置する
            // スプライトシートリストの先頭から検索し、
            // 空き地が見つかった順に追加を試みる。
            // ブロックサイズは無視する
            candidates = _filterSpriteSheetNumberByBlockSize(this, group, 0); // match all
            break;
        case TextureAtlas.CLASSIFY_COMPACT: // 可能な限り速度を維持しつつ面積をコンパクトにする
            // スプライトシートリストを先頭から検索し、
            // bs と一致するブロックサイズを持ったスプライトシートに空き地があるか検索する。
            // 見つからない場合は、classify = 0 が指定された場合と同じ条件で再検索を行う
            candidates = _filterSpriteSheetNumberByBlockSize(this, group, bs);
            if (!candidates.length) {
                candidates = _filterSpriteSheetNumberByBlockSize(this, group, 0); // match all
            }
            break;
        case TextureAtlas.CLASSIFY_ALIGN: // 可能な限りサイズを揃え整列させる
            // スプライトシートリストを先頭から検索し、
            // bs と一致するブロックサイズを持ったスプライトシートに空き地があるか検索する。
            // 見つからない場合は空き地無しとして検索を終了する
            // 結果的に、新しくスプライトシート(プロックサイズはbs)が追加され、そこに画像を追加する
            candidates = _filterSpriteSheetNumberByBlockSize(this, group, bs);
        }
        ...
        if (candidates.length) {
            // 画像を inject できるだけの隙間(空き地:FreeSpace)を検索
        ...
        if (freeSpace) {
            ...
            // 隙間が無ければ、新しいスプライトシートを追加し、画像を左上(bx:0,by:0)に配置する
...
// 画像のサイズに対して適当なブロックサイズを返す
function _getFitBlockSize(sw,   // @arg UINT32 - source width
                          sh) { // @arg UINT32 - source height
                                // @ret UINT8 - fit block size (8 to 64)
/*
    | BlockSize | TextureAtlas size | image long edge |
    |-----------|-------------------|-----------------|
    |         8 |             256px |     1 to 128px  |
    |        16 |             512px |    33 to 256px  |
    |        32 |            1024px |   129 to 512px  |
    |        64 |            2048px |   513 to 2048px |
 */
    if ( Math.max(_getBlockSize(sw, 8),  _getBlockSize(sh, 8)) <= 16 ) { // 8 * 16 = 128px
        return 8;
    }
    if ( Math.max(_getBlockSize(sw, 16), _getBlockSize(sh, 16)) <= 16) { // 16 * 16 = 256px
        return 16;
    }
    if ( Math.max(_getBlockSize(sw, 32), _getBlockSize(sh, 32)) <= 16) { // 32 * 16 = 512px
        return 32;
    }
    return 64;
}

function _getBlockSize(size, // @arg UINT32 - image size
                       bs) { // @arg UINT8  - block size
    switch (bs) {
...
function TextureAtlas_dump(ssn) { // @arg SpriteSheetNumber = undefined
    if (ssn === undefined) { // dump all
        this._maps.forEach(function(map, ssn) {
            console.log("SpriteSheetNumber: " + ssn);
            _dump(map);
        });
    } else {
