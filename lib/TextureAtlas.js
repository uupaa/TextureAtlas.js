...
//}@dev
    ...
    for (var i = 0, iz = resources.length; i < iz; ++i) {
        ...
        if (candidate.length) {
            freeSpace = _findFreeSpace(this, candidate, sw, sh); // { ssn:UINT8, bs: UINT8, bx:UINT16, by:UINT16, bw:UINT8, bh:UINT8 } or null
        }
        if (freeSpace) {
            ssn = freeSpace.ssn;
            bx  = freeSpace.bx;
        ...
        _add(this, id, source, ssn, bs, sx, sy, sw, sh, bx, by, bw, bh);
        ssna.push(ssn);
...
function _addSpriteSheet(that,        // @arg this
    ...
    if (TextureAtlas["VERBOSE"] && global["document"]) {
        var bgcolor = { "8": "lightcyan", "16": "aquamarine", "32": "turquoise", "64": "lightseagreen" }[blockSize];
