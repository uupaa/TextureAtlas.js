...
TextureAtlas["prototype"] = Object.create(TextureAtlas, {
    ...
    "add":          { "value": TextureAtlas_add         },  // TextureAtlas#add(resources:ResourceArray, group:String = "", rule:Integer = 0):void
...
function _updateCache(that,    // @arg this
    ...
    _toBlob(that, ssn, "image/png", async, function(blob) {
        var img = new Image();
    ...
    function _toBlob(that, ssn, mimeType, async, callback) {
      //var canvas = that._spriteSheets[ssn];
/*
        if (async) {
            if (canvas.toBlob) {
 ...
 */
        var ctx = that._ctxs[ssn];
        var u8c = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height).data;
        var header = _createBitmapData(u8c, ctx.canvas.width, ctx.canvas.height);

        var blob = new Blob([ header, u8c ], { "type": "image/bmp" });

        callback(blob);
    }
}

function _createBitmapData(source, width, height) {
    function _getLittleEndian(value) {
        var result = [];
    ...
    var u8 = new Uint8Array(54);
    var size1 = _getLittleEndian(bodyLength);
    var size2 = _getLittleEndian(width);
    var size3 = _getLittleEndian(height);

    u8.set([0x42, 0x4d], 0); // "BM"
    u8.set(size1, 2);
    u8.set([0x00, 0x00,
            0x00, 0x00,
            0x36, 0x00, 0x00, 0x00,
            0x28, 0x00, 0x00, 0x00], 6);
    u8.set(size2, 18);
    u8.set(size3, 22);
    u8.set([0x01, 0x00,
            0x20, 0x00,
...
function TextureAtlas_add(resources, // @arg ResourceArrayArray|ResourceObjectArray - [[source, id, rect ] or { source, id, rect }, ...]
                          ...
                          rule) {    // @arg Integer = RULE_FIND_FIRST - rule. RULE_FIND_FIRST or RULE_COMPACT or RULE_ALIGN
                                     // @resource.id     String
...
//}@dev
    ...
    for (var i = 0, iz = resources.length; i < iz; ++i) {
        ...
        if (longEdge > TextureAtlas.MAX_BLOCK_SIZE * 32) { // 2048px over?
...
function _getBlockSize(size, // @arg UINT32 - image size (long edge)
...
function _addSpriteSheet(that, // @arg this
    ...
    if (bs < TextureAtlas.MIN_BLOCK_SIZE ||
        bs > TextureAtlas.MAX_BLOCK_SIZE) {
    }
