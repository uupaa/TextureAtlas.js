...
TextureAtlas.DIRTY  = 2; // a cache is dirty
...
//  "tile":         { "value": TextureAtlas_tile        },  // TextureAtlas#tile(width:UINT16, height:UINT16, length:UINT16):TilingDataArray - [ { ssn, i, bx, by, bw, bh }, ... ]
    ...
    "updateCache":  { "value": TextureAtlas_updateCache },  // TextureAtlas#updateCache(all:Boolean = false):void
});
...
function TextureAtlas_updateCache(all) { // @arg Boolean = false - update all dirty cache or update a dirty cache
    if (this._useCache) {
        var ssna = Object.keys(this._cacheState); // [ssn, ...]
        for (var i = 0, iz = ssna.length; i < iz; ++i) {
            if (ssna[i] === TextureAtlas.DIRTY) {
...
//}@dev
    ...
    for (var i = 0, iz = resources.length; i < iz; ++i) {
        ...
        if (longEdge > TextureAtlas.MAX_BLOCK_SIZE * 32) { // over?
